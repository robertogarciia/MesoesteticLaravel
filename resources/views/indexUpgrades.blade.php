            ...
            <div class="card border-black border-2 shadow">
                <div class="card-body">
                    ...
                    <div class="d-flex flex-column align-items-start" style="border-radius:10px;padding-left:10px;">
                        @foreach (['medicamentos' => '#F94537', 'sanitaria' => '#8AE34B', 'cosmeticos' => '#3A3AD4', 'Control de calidad' => '#bfbfbf'] as $zoneName => $color)
                            <div class="d-flex align-items-center">
                                <div style="width: 20px; height: 20px; margin: 10px; padding: 10px; margin-bottom:10px; border-radius: 50%; background-color: {{ $color }};"></div>
                                <h5 class="zone-filter {{ $zone == $zoneName ? 'active-filter' : '' }}" data-zone="{{ $zoneName }}">{{ ucfirst($zoneName) }}</h5>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="d-flex flex-column alignments-start" style="border-radius:10px;padding-left:10px;">
                        <form method="GET" action="{{ route('upgrades.index') }}">
                            ...
                            <div class="col-12">
                                ...
                                <input id="start_date" type="date" name="start_date" class="form-control" value="{{ $start_date }}" required>
                            </div>
                            <div class="col-12">
                                <label for="end_date">Hasta:</label>
                                <input id="end_date" type="date" name="end_date" class="form-control" value="{{ $end_date }}" required>
                            </div>
                        ...
                        <div class="d-flex alignments-center" style="margin-top:8px;margin-left:14px;">
                            <h5 class="zone-filter {{ $zone == 'todos' ? 'active-filter' : '' }}" data-zone="todos">Limpiar Filtro</h5>
                        </div>
                    </div>
                </div>
            </div>
